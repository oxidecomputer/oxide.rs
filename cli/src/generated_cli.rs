...
impl<T: CliConfig> Cli<T> {
        ...
        ::clap::Command::new("")
            ...
            .arg(
                ::clap::Arg::new("boot-disk")
                    ...
                    .value_parser(::clap::value_parser!(types::NameOrId))
                    .required_unless_present("json-body")
                    .help("The disk the instance is configured to boot from."),
            )
            .arg(
                ::clap::Arg::new("cpu-platform")
                    .long("cpu-platform")
                    ...
                    .required_unless_present("json-body")
                    .help("The CPU platform to be used for this instance."),
            )
            .arg(
                ::clap::Arg::new("enable-jumbo-frames")
                    .long("enable-jumbo-frames")
    ...
    pub fn cli_instance_multicast_group_leave() -> ::clap::Command {
        ::clap::Command::new("")
            .arg(
                ::clap::Arg::new("instance")
                    .long("instance")
    ...
    pub fn cli_multicast_group_list() -> ::clap::Command {
        ::clap::Command::new("")
            .arg(
                ::clap::Arg::new("limit")
                    .long("limit")
        ...
        ::clap::Command::new("")
            ...
            .arg(
                ::clap::Arg::new("ip-version")
                    ...
                    .required(false)
                    .help("The IP version of the pool.\n\nThe default is IPv4."),
            )
            .arg(
                ::clap::Arg::new("name")
                    .long("name")
    ...
    pub async fn execute(
        ...
        match cmd {
            ...
            CliCommand::InstanceMulticastGroupJoin => {
                self.execute_instance_multicast_group_join(matches).await
            }
            CliCommand::InstanceMulticastGroupLeave => {
                self.execute_instance_multicast_group_leave(matches).await
            }
    ...
    pub async fn execute_probe_create(&self, matches: &::clap::ArgMatches) -> anyhow::Result<()> {
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::ProbeCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_support_bundle_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::SupportBundleCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_support_bundle_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::SupportBundleUpdate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_affinity_group_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::AffinityGroupCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_affinity_group_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::AffinityGroupUpdate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_alert_receiver_subscription_add(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::AlertSubscriptionCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_anti_affinity_group_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::AntiAffinityGroupCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_anti_affinity_group_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::AntiAffinityGroupUpdate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_auth_settings_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::SiloAuthSettingsUpdate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_disk_bulk_write_import(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::ImportBlocksBulkWrite>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_disk_finalize_import(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::FinalizeDisk>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_floating_ip_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::FloatingIpCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_floating_ip_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::FloatingIpUpdate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_floating_ip_attach(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::FloatingIpAttach>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_instance_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::InstanceUpdate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_instance_ephemeral_ip_attach(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::EphemeralIpCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_instance_multicast_group_list(
        &self,
        matches: &::clap::ArgMatches,
    ) -> anyhow::Result<()> {
        let mut request = self.client.instance_multicast_group_list();
        if let Some(value) = matches.get_one::<types::NameOrId>("instance") {
            request = request.instance(value.clone());
        }
    ...
    pub async fn execute_instance_multicast_group_join(
        &self,
        matches: &::clap::ArgMatches,
    ) -> anyhow::Result<()> {
        let mut request = self.client.instance_multicast_group_join();
        if let Some(value) = matches.get_one::<types::NameOrId>("instance") {
            request = request.instance(value.clone());
        }
        ...
        self.config
            .execute_instance_multicast_group_join(matches, &mut request)?;
        let result = request.send().await;
        match result {
            Ok(r) => {
                self.config.success_item(&r);
    ...
    pub async fn execute_instance_multicast_group_leave(
        &self,
        matches: &::clap::ArgMatches,
    ) -> anyhow::Result<()> {
        let mut request = self.client.instance_multicast_group_leave();
        if let Some(value) = matches.get_one::<types::NameOrId>("instance") {
            request = request.instance(value.clone());
        }
        ...
        self.config
            .execute_instance_multicast_group_leave(matches, &mut request)?;
        let result = request.send().await;
        match result {
            Ok(r) => {
                self.config.success_no_item(&r);
    ...
    pub async fn execute_internet_gateway_ip_address_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value =
                serde_json::from_str::<types::InternetGatewayIpAddressCreate>(&body_txt)
                    .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_internet_gateway_ip_pool_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::InternetGatewayIpPoolCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_internet_gateway_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::InternetGatewayCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_multicast_group_list(
        &self,
        matches: &::clap::ArgMatches,
    ) -> anyhow::Result<()> {
        let mut request = self.client.multicast_group_list();
        if let Some(value) = matches.get_one::<::std::num::NonZeroU32>("limit") {
            request = request.limit(value.clone());
        }
        ...
        self.config
            .execute_multicast_group_list(matches, &mut request)?;
        self.config.list_start::<types::MulticastGroupResultsPage>();
        let mut stream = futures::StreamExt::take(
            request.stream(),
            matches
                .get_one::<std::num::NonZeroU32>("limit")
        ...
        loop {
            match futures::TryStreamExt::try_next(&mut stream).await {
                ...
                Ok(None) => {
                    self.config
                        .list_end_success::<types::MulticastGroupResultsPage>();
                    return Ok(());
                }
                Ok(Some(value)) => {
                    self.config.list_item(&value);
    ...
    pub async fn execute_multicast_group_view(
        ...
        self.config
            .execute_multicast_group_view(matches, &mut request)?;
        let result = request.send().await;
        match result {
            Ok(r) => {
                self.config.success_item(&r);
    ...
    pub async fn execute_multicast_group_member_list(
        ...
        let mut request = self.client.multicast_group_member_list();
        if let Some(value) = matches.get_one::<::std::num::NonZeroU32>("limit") {
            request = request.limit(value.clone());
        }
        ...
        self.config
            .execute_multicast_group_member_list(matches, &mut request)?;
        self.config
            .list_start::<types::MulticastGroupMemberResultsPage>();
        let mut stream = futures::StreamExt::take(
            request.stream(),
    ...
    pub async fn execute_instance_network_interface_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value =
                serde_json::from_str::<types::InstanceNetworkInterfaceCreate>(&body_txt)
                    .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_instance_network_interface_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value =
                serde_json::from_str::<types::InstanceNetworkInterfaceUpdate>(&body_txt)
                    .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_sled_set_provision_policy(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::SledProvisionPolicyParams>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_networking_switch_port_lldp_config_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::LldpLinkConfig>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_networking_switch_port_apply_settings(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::SwitchPortApplySettings>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_system_ip_pool_silo_link(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::IpPoolLinkSilo>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_system_ip_pool_silo_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::IpPoolSiloUpdate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_networking_address_lot_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::AddressLotCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_networking_allow_list_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::AllowListUpdate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_networking_bfd_disable(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::BfdSessionDisable>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_networking_bfd_enable(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::BfdSessionEnable>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_networking_bgp_config_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::BgpConfigCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_networking_bgp_announce_set_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::BgpAnnounceSetCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_networking_inbound_icmp_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::ServiceIcmpConfig>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_networking_loopback_address_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::LoopbackAddressCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_networking_switch_port_settings_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::SwitchPortSettingsCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_silo_quotas_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::SiloQuotasUpdate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_system_timeseries_query(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::TimeseriesQuery>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_system_update_trust_root_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<::serde_json::Value>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_vpc_router_route_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::RouterRouteCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_vpc_router_route_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::RouterRouteUpdate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_webhook_receiver_create(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::WebhookCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_webhook_receiver_update(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::WebhookReceiverUpdate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
    ...
    pub async fn execute_webhook_secrets_add(
        ...
        if let Some(value) = matches.get_one::<std::path::PathBuf>("json-body") {
            let body_txt = std::fs::read_to_string(value)
                .with_context(|| format!("failed to read {}", value.display()))?;
            let body_value = serde_json::from_str::<types::WebhookSecretCreate>(&body_txt)
                .with_context(|| format!("failed to parse {}", value.display()))?;
            request = request.body(body_value);
        }
...
pub trait CliConfig {
    ...
    fn execute_instance_multicast_group_list(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::InstanceMulticastGroupList,
    ) -> anyhow::Result<()> {
        Ok(())
    }

    fn execute_instance_multicast_group_join(
        &self,
        matches: &::clap::ArgMatches,
    ...
    fn execute_instance_multicast_group_leave(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::InstanceMulticastGroupLeave,
    ) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    fn execute_multicast_group_list(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::MulticastGroupList,
    ) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    fn execute_multicast_group_view(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::MulticastGroupView,
    ) -> anyhow::Result<()> {
        Ok(())
    }
...
impl CliCommand {
    ...
    pub fn operation_id(&self) -> &'static str {
        match self {
            ...
            CliCommand::ProbeList => "probe_list",
            CliCommand::ProbeCreate => "probe_create",
            CliCommand::ProbeView => "probe_view",
            CliCommand::ProbeDelete => "probe_delete",
            CliCommand::SupportBundleList => "support_bundle_list",
            CliCommand::SupportBundleCreate => "support_bundle_create",
            CliCommand::SupportBundleView => "support_bundle_view",
            CliCommand::SupportBundleUpdate => "support_bundle_update",
            CliCommand::SupportBundleDelete => "support_bundle_delete",
            CliCommand::SupportBundleDownload => "support_bundle_download",
            CliCommand::SupportBundleHead => "support_bundle_head",
            CliCommand::SupportBundleDownloadFile => "support_bundle_download_file",
            CliCommand::SupportBundleHeadFile => "support_bundle_head_file",
            CliCommand::SupportBundleIndex => "support_bundle_index",
            CliCommand::LoginSaml => "login_saml",
            CliCommand::AffinityGroupList => "affinity_group_list",
            CliCommand::AffinityGroupCreate => "affinity_group_create",
            CliCommand::AffinityGroupView => "affinity_group_view",
            CliCommand::AffinityGroupUpdate => "affinity_group_update",
            CliCommand::AffinityGroupDelete => "affinity_group_delete",
            CliCommand::AffinityGroupMemberList => "affinity_group_member_list",
            CliCommand::AffinityGroupMemberInstanceView => "affinity_group_member_instance_view",
            CliCommand::AffinityGroupMemberInstanceAdd => "affinity_group_member_instance_add",
            CliCommand::AffinityGroupMemberInstanceDelete => {
                "affinity_group_member_instance_delete"
            }
            CliCommand::AlertClassList => "alert_class_list",
            CliCommand::AlertReceiverList => "alert_receiver_list",
            CliCommand::AlertReceiverView => "alert_receiver_view",
            CliCommand::AlertReceiverDelete => "alert_receiver_delete",
            CliCommand::AlertDeliveryList => "alert_delivery_list",
            ...
            CliCommand::AntiAffinityGroupView => "anti_affinity_group_view",
            CliCommand::AntiAffinityGroupUpdate => "anti_affinity_group_update",
            CliCommand::AntiAffinityGroupDelete => "anti_affinity_group_delete",
            CliCommand::AntiAffinityGroupMemberList => "anti_affinity_group_member_list",
            CliCommand::AntiAffinityGroupMemberInstanceView => {
            ...
            CliCommand::AuthSettingsView => "auth_settings_view",
            CliCommand::AuthSettingsUpdate => "auth_settings_update",
            ...
            CliCommand::DiskBulkWriteImport => "disk_bulk_write_import",
            CliCommand::DiskBulkWriteImportStart => "disk_bulk_write_import_start",
            CliCommand::DiskBulkWriteImportStop => "disk_bulk_write_import_stop",
            CliCommand::DiskFinalizeImport => "disk_finalize_import",
            CliCommand::ExternalSubnetList => "external_subnet_list",
            CliCommand::ExternalSubnetCreate => "external_subnet_create",
            CliCommand::ExternalSubnetView => "external_subnet_view",
            CliCommand::ExternalSubnetUpdate => "external_subnet_update",
            CliCommand::ExternalSubnetDelete => "external_subnet_delete",
            CliCommand::ExternalSubnetAttach => "external_subnet_attach",
            CliCommand::ExternalSubnetDetach => "external_subnet_detach",
            CliCommand::FloatingIpList => "floating_ip_list",
            CliCommand::FloatingIpCreate => "floating_ip_create",
            CliCommand::FloatingIpView => "floating_ip_view",
            CliCommand::FloatingIpUpdate => "floating_ip_update",
            CliCommand::FloatingIpDelete => "floating_ip_delete",
            CliCommand::FloatingIpAttach => "floating_ip_attach",
            CliCommand::FloatingIpDetach => "floating_ip_detach",
            CliCommand::GroupList => "group_list",
            CliCommand::GroupView => "group_view",
            ...
            CliCommand::ImageDemote => "image_demote",
            CliCommand::ImagePromote => "image_promote",
            CliCommand::InstanceList => "instance_list",
            CliCommand::InstanceCreate => "instance_create",
            CliCommand::InstanceView => "instance_view",
            CliCommand::InstanceUpdate => "instance_update",
            CliCommand::InstanceDelete => "instance_delete",
            CliCommand::InstanceAffinityGroupList => "instance_affinity_group_list",
            CliCommand::InstanceAntiAffinityGroupList => "instance_anti_affinity_group_list",
            ...
            CliCommand::InstanceEphemeralIpAttach => "instance_ephemeral_ip_attach",
            CliCommand::InstanceEphemeralIpDetach => "instance_ephemeral_ip_detach",
            CliCommand::InstanceExternalSubnetList => "instance_external_subnet_list",
            CliCommand::InstanceMulticastGroupList => "instance_multicast_group_list",
            CliCommand::InstanceMulticastGroupJoin => "instance_multicast_group_join",
            CliCommand::InstanceMulticastGroupLeave => "instance_multicast_group_leave",
            ...
            CliCommand::InstanceSshPublicKeyList => "instance_ssh_public_key_list",
            CliCommand::InstanceStart => "instance_start",
            CliCommand::InstanceStop => "instance_stop",
            CliCommand::InternetGatewayIpAddressList => "internet_gateway_ip_address_list",
            CliCommand::InternetGatewayIpAddressCreate => "internet_gateway_ip_address_create",
            CliCommand::InternetGatewayIpAddressDelete => "internet_gateway_ip_address_delete",
            CliCommand::InternetGatewayIpPoolList => "internet_gateway_ip_pool_list",
            CliCommand::InternetGatewayIpPoolCreate => "internet_gateway_ip_pool_create",
            CliCommand::InternetGatewayIpPoolDelete => "internet_gateway_ip_pool_delete",
            CliCommand::InternetGatewayList => "internet_gateway_list",
            CliCommand::InternetGatewayCreate => "internet_gateway_create",
            CliCommand::InternetGatewayView => "internet_gateway_view",
            CliCommand::InternetGatewayDelete => "internet_gateway_delete",
            ...
            CliCommand::CurrentUserView => "current_user_view",
            CliCommand::CurrentUserAccessTokenList => "current_user_access_token_list",
            CliCommand::CurrentUserAccessTokenDelete => "current_user_access_token_delete",
            CliCommand::CurrentUserGroups => "current_user_groups",
            CliCommand::CurrentUserSshKeyList => "current_user_ssh_key_list",
            CliCommand::CurrentUserSshKeyCreate => "current_user_ssh_key_create",
            CliCommand::CurrentUserSshKeyView => "current_user_ssh_key_view",
            CliCommand::CurrentUserSshKeyDelete => "current_user_ssh_key_delete",
            CliCommand::SiloMetric => "silo_metric",
            CliCommand::MulticastGroupList => "multicast_group_list",
            CliCommand::MulticastGroupView => "multicast_group_view",
            CliCommand::MulticastGroupMemberList => "multicast_group_member_list",
            ...
            CliCommand::RackMembershipStatus => "rack_membership_status",
            CliCommand::RackMembershipAbort => "rack_membership_abort",
            CliCommand::RackMembershipAddSleds => "rack_membership_add_sleds",
            CliCommand::SledList => "sled_list",
            CliCommand::SledView => "sled_view",
            CliCommand::SledPhysicalDiskList => "sled_physical_disk_list",
            CliCommand::SledInstanceList => "sled_instance_list",
            CliCommand::SledSetProvisionPolicy => "sled_set_provision_policy",
            CliCommand::SledListUninitialized => "sled_list_uninitialized",
            CliCommand::NetworkingSwitchPortList => "networking_switch_port_list",
            CliCommand::NetworkingSwitchPortLldpConfigView => {
            ...
            CliCommand::NetworkingSwitchPortStatus => "networking_switch_port_status",
            CliCommand::SwitchList => "switch_list",
            CliCommand::SwitchView => "switch_view",
            ...
            CliCommand::SystemIpPoolList => "system_ip_pool_list",
            CliCommand::SystemIpPoolCreate => "system_ip_pool_create",
            ...
            CliCommand::SystemIpPoolSiloList => "system_ip_pool_silo_list",
            CliCommand::SystemIpPoolSiloLink => "system_ip_pool_silo_link",
            ...
            CliCommand::NetworkingAddressLotView => "networking_address_lot_view",
            CliCommand::NetworkingAddressLotDelete => "networking_address_lot_delete",
            CliCommand::NetworkingAddressLotBlockList => "networking_address_lot_block_list",
            CliCommand::NetworkingAllowListView => "networking_allow_list_view",
            CliCommand::NetworkingAllowListUpdate => "networking_allow_list_update",
            CliCommand::NetworkingBfdDisable => "networking_bfd_disable",
            ...
            CliCommand::NetworkingBgpAnnounceSetUpdate => "networking_bgp_announce_set_update",
            CliCommand::NetworkingBgpAnnounceSetDelete => "networking_bgp_announce_set_delete",
            CliCommand::NetworkingBgpAnnouncementList => "networking_bgp_announcement_list",
            CliCommand::NetworkingBgpExported => "networking_bgp_exported",
            CliCommand::NetworkingBgpImported => "networking_bgp_imported",
            CliCommand::NetworkingBgpMessageHistory => "networking_bgp_message_history",
            CliCommand::NetworkingBgpStatus => "networking_bgp_status",
            CliCommand::NetworkingInboundIcmpView => "networking_inbound_icmp_view",
            CliCommand::NetworkingInboundIcmpUpdate => "networking_inbound_icmp_update",
            ...
            CliCommand::NetworkingSwitchPortSettingsView => "networking_switch_port_settings_view",
            CliCommand::SystemPolicyView => "system_policy_view",
            CliCommand::SystemPolicyUpdate => "system_policy_update",
            CliCommand::ScimTokenList => "scim_token_list",
            CliCommand::ScimTokenCreate => "scim_token_create",
            CliCommand::ScimTokenView => "scim_token_view",
            CliCommand::ScimTokenDelete => "scim_token_delete",
            CliCommand::SystemQuotasList => "system_quotas_list",
            ...
            CliCommand::SiloIpPoolList => "silo_ip_pool_list",
            CliCommand::SiloPolicyView => "silo_policy_view",
            CliCommand::SiloPolicyUpdate => "silo_policy_update",
            CliCommand::SiloQuotasView => "silo_quotas_view",
            CliCommand::SiloQuotasUpdate => "silo_quotas_update",
            CliCommand::SiloSubnetPoolList => "silo_subnet_pool_list",
            CliCommand::SystemSubnetPoolList => "system_subnet_pool_list",
            CliCommand::SystemSubnetPoolCreate => "system_subnet_pool_create",
            CliCommand::SystemSubnetPoolView => "system_subnet_pool_view",
            CliCommand::SystemSubnetPoolUpdate => "system_subnet_pool_update",
            CliCommand::SystemSubnetPoolDelete => "system_subnet_pool_delete",
            CliCommand::SystemSubnetPoolMemberList => "system_subnet_pool_member_list",
            CliCommand::SystemSubnetPoolMemberAdd => "system_subnet_pool_member_add",
            CliCommand::SystemSubnetPoolMemberRemove => "system_subnet_pool_member_remove",
            CliCommand::SystemSubnetPoolSiloList => "system_subnet_pool_silo_list",
            CliCommand::SystemSubnetPoolSiloLink => "system_subnet_pool_silo_link",
            CliCommand::SystemSubnetPoolSiloUpdate => "system_subnet_pool_silo_update",
            CliCommand::SystemSubnetPoolSiloUnlink => "system_subnet_pool_silo_unlink",
            CliCommand::SystemSubnetPoolUtilizationView => "system_subnet_pool_utilization_view",
            CliCommand::SystemTimeseriesQuery => "system_timeseries_query",
            CliCommand::SystemTimeseriesSchemaList => "system_timeseries_schema_list",
            CliCommand::SystemUpdateRecoveryFinish => "system_update_recovery_finish",
            CliCommand::SystemUpdateRepositoryList => "system_update_repository_list",
            CliCommand::SystemUpdateRepositoryUpload => "system_update_repository_upload",
            CliCommand::SystemUpdateRepositoryView => "system_update_repository_view",
            CliCommand::SystemUpdateStatus => "system_update_status",
            CliCommand::TargetReleaseUpdate => "target_release_update",
            CliCommand::SystemUpdateTrustRootList => "system_update_trust_root_list",
            CliCommand::SystemUpdateTrustRootCreate => "system_update_trust_root_create",
            CliCommand::SystemUpdateTrustRootView => "system_update_trust_root_view",
            CliCommand::SystemUpdateTrustRootDelete => "system_update_trust_root_delete",
            CliCommand::SiloUserList => "silo_user_list",
            CliCommand::SiloUserView => "silo_user_view",
            CliCommand::UserBuiltinList => "user_builtin_list",
            CliCommand::UserBuiltinView => "user_builtin_view",
            CliCommand::SiloUtilizationList => "silo_utilization_list",
            CliCommand::SiloUtilizationView => "silo_utilization_view",
            CliCommand::TimeseriesQuery => "timeseries_query",
            CliCommand::UserList => "user_list",
            CliCommand::UserView => "user_view",
            CliCommand::UserTokenList => "user_token_list",
            CliCommand::UserLogout => "user_logout",
            CliCommand::UserSessionList => "user_session_list",
            CliCommand::UtilizationView => "utilization_view",
            ...
            CliCommand::WebhookReceiverCreate => "webhook_receiver_create",
            CliCommand::WebhookReceiverUpdate => "webhook_receiver_update",
            CliCommand::WebhookSecretsList => "webhook_secrets_list",
            CliCommand::WebhookSecretsAdd => "webhook_secrets_add",
            CliCommand::WebhookSecretsDelete => "webhook_secrets_delete",
        }
    }
}
